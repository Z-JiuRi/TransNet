...
- 训练集读取 `train{scenario}.pt`，验证集读取 `val{scenario}.pt`，测试集读取
  `test{scenario}.pt`。例如 `--scenario in` 时读取 `trainin.pt`、`valin.pt`
  和 `testin.pt`。三者都会被转换为 `float32`，并按
  `(N, channel, nt, nc)` 校验或 reshape。
...
- `python main.py --data-dir ./COST2100 --scenario in --epochs 400 --batch-size 200 --workers 0 --cr 4 --nt 32 --nc 32 --scheduler const --gpu 0`：从头开始训练。
- `python main.py --data-dir ./COST2100 --scenario in --pretrained ./checkpoints/4_in.pth --evaluate --batch-size 200 --workers 0 --cr 4 --nt 32 --nc 32 --cpu`：使用 checkpoint 进行评估。
