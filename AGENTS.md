...
`32` 是天线维，另一个 `32` 是截取后的延迟/频域维。因此单样本展平后的特征数为
`2 * 32 * 32 = 2048`。

数据从 `--data-dir` 指向的 COST2100 目录进入 `dataloader/cost2100.py`：

- 训练集读取 `DATA_Htrain{scenario}.mat` 中的 `HT`，验证集读取
  `DATA_Hval{scenario}.mat` 中的 `HT`，测试集读取 `DATA_Htest{scenario}.mat`
  中的 `HT`。三者都会被转换为 `float32`，并 reshape 为
  `(N, 2, 32, 32)`。
...
例如 `--cr 4` 对应压缩率 `1/4`，压缩后的码字长度是 `2048 / 4 = 512`。

模型前向传播的主要维度变化如下：

- 输入 `src`: `(B, 2, 32, 32)`。
- `feature_shape = (2048 // d_model, d_model)`。默认 `d_model=64`，所以
  `feature_shape=(32, 64)`。要求 `2048 % d_model == 0`。
- `src.view(-1, 2048 // d_model, d_model)` 将输入展为 Transformer 序列：
  默认是 `(B, 32, 64)`，其中序列长度为 `32`，每个 token 的特征维为 `64`。
- Encoder 输入和输出维度都保持 `(B, 32, 64)`。每层自注意力使用 `nhead=2`，
  单头维度是 `d_model / nhead = 32`。
- Encoder 输出展平成 `(B, 2048)`，经过 `fc_encoder: Linear(2048, 2048/cr)`
  得到压缩码字 `(B, 2048/cr)`。例如 `cr=4` 时是 `(B, 512)`。
- `fc_decoder: Linear(2048/cr, 2048)` 将码字恢复为 `(B, 2048)`，再 reshape 回
  `(B, 32, 64)` 作为 Decoder 输入。
...
- `python main.py --data-dir ./COST2100 --scenario in --epochs 400 --batch-size 200 --workers 0 --cr 4 --scheduler const --gpu 0`：从头开始训练。
- `python main.py --data-dir ./COST2100 --scenario in --pretrained ./checkpoints/4_in.pth --evaluate --batch-size 200 --workers 0 --cr 4 --cpu`：使用 checkpoint 进行评估。
...
不要在可复用代码中硬编码本地绝对路径。路径应通过 `--data-dir`、`--pretrained`
和 `--resume` 等 CLI 参数传入。下载的数据集和 checkpoint 应保留在版本控制之外。
